...
import com.simlike.simscript.generator.ui.js.ReturnFlags
import com.simlike.simscript.generator.ui.js.StatementsGenerator
...
import com.simlike.simscript.ui.simUiDsl.AndExpression
...
import com.simlike.simscript.ui.simUiDsl.Comparator
import com.simlike.simscript.ui.simUiDsl.ComparisonExpression
import com.simlike.simscript.ui.simUiDsl.DateNowExpression
import com.simlike.simscript.ui.simUiDsl.DateTodayExpression
...
import com.simlike.simscript.ui.simUiDsl.OrExpression
...
class ExpressionsGeneratorImpl implements ExpressionsGenerator {
	 ...
	 */
	override asObservableJs(Expression it)	{	asObservableJs_ }

	/**
	 * @return JS code that evaluates the given {@link Expression} as a(n unwrapped) value.
	 */
	override asPlainJs(Expression it)		{	asPlainJs_ }
	...
	def private dispatch asObservableJs_(TernaryExpression it)	'''new Item(«asPlainJs_»)'''
	def private dispatch asPlainJs_(TernaryExpression it)		'''(«guard.asPlainJs_» ? «thenExpr.asPlainJs_» : «elseExpr.asPlainJs_»)'''

	def private dispatch asObservableJs_(OrExpression it)	'''new Item(«asPlainJs_»)'''
	def private dispatch asPlainJs_(OrExpression it)		'''(«leftOperand.asPlainJs_» || «rightOperand.asPlainJs_»)'''

	def private dispatch asObservableJs_(AndExpression it)	'''new Item(«asPlainJs_»)'''
	def private dispatch asPlainJs_(AndExpression it)		'''(«leftOperand.asPlainJs_» && «rightOperand.asPlainJs_»)'''
	 ...
	 */
	def private dispatch asObservableJs_(ComparisonExpression it)	'''new Item(«asPlainJs_»)'''
	def private dispatch asPlainJs_(ComparisonExpression it) {
	 ...
	 */
	def private dispatch asObservableJs_(ArithmeticBinaryOperatorExpression it)	'''new Item(«asPlainJs_»)'''
	def private dispatch asPlainJs_(ArithmeticBinaryOperatorExpression it)		'''(«leftOperand.asPlainJs_» «operatorAsJs» «rightOperand.asPlainJs_»)'''
	...
	def private dispatch asObservableJs_(NotExpression it)						'''new Item(«asPlainJs_»)'''
	def private dispatch asPlainJs_(NotExpression it)							'''(! «operand.asPlainJs_»)'''
	 ...
	 */
	def private dispatch asObservableJs_(FeatureAccessExpression it)		'''«previous.asObservableJs_».get«feature.name.toFirstUpper»()'''
	def private dispatch asPlainJs_(FeatureAccessExpression it)				'''«asObservableJs_».unwrap()'''

	def private dispatch asObservableJs_(StringLiteralExpression it)		'''new Item(«asPlainJs_»)'''
	def private dispatch asPlainJs_(StringLiteralExpression it)				'''"«value.escapeJS»"'''
	def private dispatch asObservableJs_(BooleanLiteralExpression it)		'''new Item(«asPlainJs_»)'''
	def private dispatch asPlainJs_(BooleanLiteralExpression it)			'''«literal.literal»'''
	def private dispatch asObservableJs_(IntegerLiteralExpression it)		'''new Item(«asPlainJs_»)'''
	def private dispatch asPlainJs_(IntegerLiteralExpression it)			'''«value»'''
	def private dispatch asObservableJs_(NumberLiteralExpression it)		'''new Item(«asPlainJs_»)'''
	def private dispatch asPlainJs_(NumberLiteralExpression it)				'''«value»'''
	def private dispatch asObservableJs_(ListLiteralExpression it)			'''new List(«asPlainJs_»)'''
	def private dispatch asPlainJs_(ListLiteralExpression it)				'''[«FOR m : members SEPARATOR ', '»«m.asPlainJs_»«ENDFOR»]'''
	def private dispatch asObservableJs_(EnumerationLiteralExpression it)	'''new Item(«asPlainJs_»)'''
	def private dispatch asPlainJs_(EnumerationLiteralExpression it)		'''"«literal.name»"'''	// FIXME  this is probably incorrect

	/**
	 * Dependencies/Assumptions:
	 * 1.)	assumes that CallbackExpressions in TimeSlotElements and CalendarElements accept selection as parameter.
	 */
	def private dispatch asObservableJs_(CallbackExpression it)			{ asPlainJs_ }	// note: Callbacks don't return values, so they are not observable
	def private dispatch asPlainJs_(CallbackExpression it) {
	 ...
	 */
	def private dispatch asObservableJs_(CallbackResponseExpression it)			'''response'''
	def private dispatch asPlainJs_(CallbackResponseExpression it)				'''«asObservableJs_».unwrap()'''
	def private dispatch asObservableJs_(CallbackErrorResponseExpression it)	'''error'''
	def private dispatch asPlainJs_(CallbackErrorResponseExpression it)			'''«asObservableJs_».unwrap()'''
	def private dispatch asObservableJs_(SelectionExpression it)				'''selection'''
	def private dispatch asPlainJs_(SelectionExpression it)						'''«asObservableJs_».unwrap()'''

	/**
	 * Dependencies/Assumptions:
	 * 1.)	During initialisation no databinding should be done.
	 */
	def private dispatch asObservableJs_(StructureCreationExpression it)		'''new Item(«asPlainJs_»)'''
	def private dispatch asPlainJs_(StructureCreationExpression it) {
	...
	def private dispatch asObservableJs_(ReferenceExpression it)		 { it.ref.jsName }
	...
	def private dispatch asObservableJs_(DateNowExpression it)			'''new Item(«asPlainJs_»)'''
	def private dispatch asPlainJs_(DateNowExpression it)				'''new Date()'''
	def private dispatch asObservableJs_(DateTodayExpression it)		'''new Item(«asPlainJs_»)'''
	def private dispatch asPlainJs_(DateTodayExpression it)				'''DateUtil.today()'''

	def private dispatch asObservableJs_(BuiltinFunctionExpression it) {
		switch function {
			...
			case SORT:			logProblemAndReturnJSComment('''built in function «function.name» only works in ExpressionsGenerator.asPlainJs_''')
			case IS_VALID:	'''new Item(«asPlainJs_»)'''
			case ID:		'''new Item(«asPlainJs_»)'''
			default:
				logProblemAndReturnJSComment('''built-in function «function.name» not handled in ExpressionsGenerator.asObservableJs_''')
		}
	}
	def private dispatch asPlainJs_(BuiltinFunctionExpression it) {
		switch function {
			...
			default:
				logProblemAndReturnJSComment('''built-in function «function.name» not handled in ExpressionsGenerator.asPlainJs_''')
		}
	}
	 ...
	 */
	def private dispatch asObservableJs_(MethodCallExpression it)		'''«IF method.function»new Item(«asPlainJs_»)«ELSE»«asPlainJs_»«ENDIF»'''	// note: method calls not observable
	def private dispatch asPlainJs_(MethodCallExpression it)			'''«method.jsName»(«functionSignature(method.definition.parameters, it.arguments)»)'''


	def private dispatch asObservableJs_(DecisionExpression it)			'''new Item(«asPlainJs_»)'''
	def private dispatch asPlainJs_(DecisionExpression it)
		'''
		function() {
	...
	def private dispatch asObservableJs_(Void it)						{ logProblemAndReturnJSComment("ExpressionsGenerator.asObservableJs_ called with null") }
	def private dispatch asPlainJs_(Void it)							{ logProblemAndReturnJSComment("ExpressionsGenerator.asPlainJs_ called with null") }

	def private dispatch asObservableJs_(EObject it)					{ it.logProblemAndReturnJSComment("ExpressionsGenerator.asObservableJs_ called with unhandled type " + eClass.name) }
	def private dispatch asPlainJs_(EObject it)							{ it.logProblemAndReturnJSComment("ExpressionsGenerator.asPlainJs_ called with unhandled type " + eClass.name) }
