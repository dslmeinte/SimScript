...
DateUtil = {
	apiDateFormat	: "dayOfMonth:monthInYear:yearAD:hoursInDay:minutesInHour:secondsInHour", //Seconds currently unused
 ...
 * A Type extends Observable and is a wrapper for custom objects.
 * All name:value pairs of the typeMap are added as private variables
 * of this Type and getters and setters are created.
 * If the value is a Function, this Type will just assign it to this.name instead.
 * The private variables are initialized to the value if it is not an Observable.
 * This is done because we assume that any non-Observable property is a simple type
 * like a String, Boolean or Num and the value of the name:value pair is 
 * assumed to be the default value (not that null is perfectly acceptable as a value).
 * Any Observable, however
 * 
 * If the value is not a Function the following will happen as well:
 * If the value is Observable, this Type will Observe the value and notify
 * all of this Type's Observers if it changes. 
 * This implicitly makes all of the Observers of this type Observers
 * of the value as well.
 * 
 * If the value from the name:value is not Observable, a databindName(element, events) 
 * function is created, which can be called to bind the value to a jQuery element.
 * This means that the element's value will be updated to getName() when the
 * setName(newValue) function is called on this Type.
 * Also, when any of the given events are triggered, setName(element.val()) will be called.
 */
function defineType(typeMap) {
	function Type() {
		// Required for Observer update function.
		var self = this;
		
		// Holder for non-function properties.
		var _data = {};
		...
		for (var prop in typeMap) {
			if (typeMap.hasOwnProperty(prop)) {
				if($.isFunction(typeMap[prop])) {
					// No reactiveness for functions yet.
					this.prop = typeMap[prop];
				} else {
					_data[prop]=typeMap[prop];
					
					// Define getter and setter functions.
					var propFirstUpper = prop.replace(/(.)/, function(match){return match.toUpperCase();}); 
					this['get' + propFirstUpper] = createGetter(prop);
					this['set' + propFirstUpper] = createSetter(prop);
					// If it's not Observable, let's hope it's not mutable.
					// Otherwise we can't guarantee reactiveness.
					if (!(typeMap[prop] instanceof Observable)) {
						this['databind' + propFirstUpper] = createDatabind(prop);
					} else {
						_data[prop]=null;
					}
				}
		...
		function createSetter(propName) {
			return function(newVal) {
				if (newVal !== _data[propName]) {
					...
					// Observe the new property if possible
					if (_data[prop] instanceof Observable) {
						_data[prop].addObserver(updateFunction);
					}
				}
			};
		}
		...
		/** 
		 * Returns a function(elementJQuery, eventsArray),
		 * which will bind the given non-Observable property to the JQuery element.
		 * 
		 * This means that when the element's events from eventsArray are triggered,
		 * the setter of this property is called with elementJQuery.val().
		 * When the setter of this property is called and the property changes,
		 * the value of the element will be set to the new value of the property.
		 * 
		 * @see Type.bindToElement
		 */  
		function createDatabind(propName) {
			var firstUpper = propName.replace(/(.)/, function(match){return match.toUpperCase();});
			return function(elementJQuery, eventsArray) {
				self.bindToElement(
						elementJQuery, 
						function(val){this['set' + firstUpper](val);}, 
						function(type){return type['get' + firstUpper]();}, 
						eventsArray);
			};
		}
		
		/**
		 * Binds this Type's data to the given jQuery element's value.
		 * Used to bind a single element's value to an entire Type.
		 * 
		 * Whenever this Type changes, the toValueFunc is called with this Type as its argument. 
		 * The result of the call will be put in the element's value by using the jQuery val function.
		 * 
		 * A listener will be bound to the element by using the jQuery bind function.
		 * It will listen to all events specified in eventsArray. 
		 * Note that these events are jQuery event names (e.g. 'click' and NOT 'onclick').
		 * eventsArray is optional and defaults to ['change', 'keyup'].
		 * 
		 * When the value of the element changes, the fromValueFunc will be called with the element's
		 * new value as its argument.
		 * In the fromValueFunc the element's new value can be used to update this Type's attributes .
		 * The 'this' keyword in the fromValueFunc will refer to this Type.
		 */
		this.bindToElement = function(elementJQuery, fromValueFunc, toValueFunc, eventsArray) {
			if (eventsArray === undefined) {
				eventsArray = ['change', 'keyup'];
			}
			self.addObserver(function() {
				elementJQuery.val(toValueFunc(self));
			});
			function listener(e){
				// inside a listener the 'this' keyword refers to the DOM HTML-element
				fromValueFunc.call(self, $(this).val());
			}
			for (var i = 0; i < eventsArray.length; i++) {
				elementJQuery.bind(eventsArray[i], listener);
			}
		};
		...
		this.toData = function() {
			...
			for (var i in _data) {
				...
				if (_data.hasOwnProperty(i)) {
					if (_data[i] instanceof Observable && $.isFunction(_data[i].toData)) {
						obj[i] = _data[i].toData();
					} else if (!$.isFunction(_data[i])) {
						// skip functions
						obj[i] = _data[i];
					}
				}
		...
		// Make sure new instances of this Type get a new Observable as prototype.
		Type.prototype = new Observable();
	}
	// Make Types extend Observable.
	Type.prototype = new Observable();
	return Type;
}
 ...
 * 
 * @param literal a SimpleLiteral (see isSimpleLiteral) to be wrapped.
 */
...
/**
 * Checks if a given object is a number by testing if the
 * toFixed method is present.
 * 
 * Note that this will cause wrong results when n has a toFixed function,
 * but is not actually a number
 * @param n the number to test.
 * @returns {Boolean} true iff n has a toFixed function.
 */
function isNum(n) {
	return n != null && $.isFunction(n.toFixed);
}
